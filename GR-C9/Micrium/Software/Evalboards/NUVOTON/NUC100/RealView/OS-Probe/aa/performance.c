	...
	if((System.Dsp.Mode == CHECK_MODE)||(System.Dsp.Mode == PCCAM_MODE))
	{
		switch(NUM)
		{
			...
			case X0 : if(System.Dsp.CheckMode.Zoom != X0 )
					  {
					//	Dsp_Send_Data(0x2C,0x22,0,0,D_NON); 
					    Select_Send(0x30); 	  
			...
			case X1 :  if(System.Dsp.CheckMode.Zoom != X1 )
					...
					//	Dsp_Send_Data(0x2C,0x52,20,150,D_NON); 	
						Dsp_Send_Data(0x2C,0x52,0,0,D_NON); 	
...
void Sensor1()
{
	if(System.Dsp.Sensor != SENSOR1)
		{
		//	Select_Send(0x70);
			Dsp_Send_Data(0x70,0x00,0x10,0,0);
		//	Scen_Send(0x70,0x00);
			OSTimeDly(150); 
			Scen_Send(0xC1,0x60);
			OSTimeDly(5); 
...
void Sensor2()
{
    ...
    //	Select_Send(0x71);
	//	Scen_Send(0x71,0x00);
		Dsp_Send_Data(0x71,0x00,0x10,0,0);
...
//	}
	...
	if(System.Dsp.Mode == VIEW_MODE)
	{
		SHOW_SCREEN_T_W(0xE3,0x05,WcharToChar(L"删除所有照片?"));	//SHOW_SCREENW(0xE8, WcharToChar(L"签发数据采集,请稍候...") );	
		...
		{		
			...
			if(key)
			{
				if(KeyValue == key)
				{
					SHOW_SCREEN_T_W(0xE3,0x05,WcharToChar(L"删除中,请稍候..."));		
				 // 	SHOW_SCREENW(0xE8, WcharToChar(L"删除中...") );

					if(System.PhotoNum != 0)
					{
				 		System.PhotoNum = 0 ;  //4张照片
						System.Sys.SaveInfoFlag  = 0x01  ;
					}
